...
public class TokenValidatorFilterProcess extends BasicAuthenticationFilter {
	

	public TokenValidatorFilterProcess(AuthenticationManager authenticationManager) {
	...
	private UsernamePasswordAuthenticationToken getAuthenticationToken(String token) {
		String usuario = JWT.require(Algorithm.HMAC512(SecurityConstsICarros.getTokenSecret())).build().verify(token).getSubject();
		if (usuario == null) {
			return null;
		}
